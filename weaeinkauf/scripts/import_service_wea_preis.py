...
from weaeinkauf.models import Quelle, QuelleDoc, ServicePreis, ServicePreisVerguetung, WeaPreis, PreisKondition, WeaFundament, WeaDetail
import csv
import os 
...
def run():
    ...
    with open(file_path_quelle, 'r') as file:
        ...
        for row in reader:
            ...
            quelle = Quelle(alt_id=alt_id, name=name, quellendatum=quellendatum, bemerkung=bemerkung, created_by=created_by, created_at=created_at)
    ...
    with open(file_path_weaDetail,'r') as file:
        ...
        for row in reader:
            ...
            if weaPreisID in wea_Preis:
                ...
                if auslaufdatum != "":
                    weaDetail = WeaDetail.objects.create(name=name, weaPreisID=weaPreisID, naebenhoehe=naebenhoehe, turmtyp=turmtyp, auslaufdatum=auslaufdatum, 
                    ...
                    weaDetail = WeaDetail.objects.create(name=name, weaPreisID=weaPreisID, naebenhoehe=naebenhoehe, turmtyp=turmtyp, 
                                                         genehmigungsunterlagen=genehmigungsunterlagen, windklasse=windklasse, gesamthoehe=gesamthoehe, weaPreis=wea_Preis[weaPreisID])
            else:
